...
if (! isset($argv[1])) {
    exit("❌ Error: Harap masukkan nominal saldo yang diinginkan.\nContoh: php set_balance_user.php 1000000\n");
}
...
if (! $balance) {
    exit("❌ Wallet tidak ditemukan!\n");
}

if ($balance->tenant_id != $userId) {
    exit("❌ Security Error: Wallet ini milik tenant {$balance->tenant_id}, bukan tenant $userId.\n");
}
...
if ($difference == 0) {
    exit("✅ Saldo sudah sesuai. Tidak ada perubahan.\n");
}
...
if (! $category) {
    exit("❌ Error: User tidak memiliki kategori apapun. Buat kategori dulu.\n");
}
