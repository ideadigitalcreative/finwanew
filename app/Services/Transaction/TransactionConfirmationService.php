...
class TransactionConfirmationService
{
    ...
    public function sendConfirmation(array $transactions, bool $needsReview = false, ?array $currentSentiment = null, ?string $currentSuggestion = null): void
    {
        ...
        if (!$needsReview && $currentSentiment && isset($currentSentiment['mood'])) {
             ...
             // Mapping
             $targetMood = $mood;
             if ($mood === 'positive') $targetMood = 'happy';
             if ($mood === 'negative') $targetMood = 'sad';
        ...
        foreach ($transactions as $index => $transaction) {
            ...
            // Show wallet not found warning if exists
            if ($count === 1 && isset($transaction->wallet_not_found_warning)) {
                $reply .= "⚠️ Dompet {$transaction->wallet_not_found_warning} tidak ditemukan, menggunakan dompet default\n";
            }
     ...
     * LINES: 4399-4490
     */
    public function sendReceiptConfirmation(Transaction $transaction, array $items, ?string $merchant): void
    {
        $amount = number_format($transaction->amount, 0, ',', '.');
        ...
        $reply .= "📅 " . $transactionDate->format('d M Y') . "\n\n";
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $item) {
                ...
                // Skip invalid items
                if (empty($nama) || $harga <= 0) continue;
                if ($harga > 50000000) continue; // Skip unrealistic prices per item (>50jt)
                if (strlen($nama) < 2) continue; // Too short
                
                // Less strict filtering
                if (preg_match('/^[0-9\s\.\,\-\:\/]+$/', $nama)) continue; // Skip if ONLY numbers/symbols
                if (stripos($nama, 'telp') !== false && strlen($nama) > 20) continue; // Phone numbers
                if (stripos($nama, 'jl.') !== false && stripos($nama, 'jalan') !== false) continue; // Likely address
            ...
            // Display max 10 items for clearer detail
            if (!empty($validItems)) {
                $reply .= "📦 *Daftar Belanja:* (" . count($validItems) . " item)\n\n";
                $displayItems = array_slice($validItems, 0, 10);
                foreach ($displayItems as $index => $item) {
                    $nama = $item['nama'];
                    $harga = number_format($item['harga'], 0, ',', '.');
                    $num = $index + 1;
                    // Format: 1. Product Name (Rp 15.000)
                    $reply .= "  {$num}. {$nama} _(Rp {$harga})_\n";
                }
                if (count($validItems) > 10) {
                    $remaining = count($validItems) - 10;
                    $reply .= "  _+ {$remaining} item lainnya..._\n";
                }
                $reply .= "\n";
            }
        }
