...
class FinancialQueryHandler
{
    ...
    protected function handleCheckBalance(): void
    {
        if ($this->handleCheckBalanceCallback) {
            call_user_func($this->handleCheckBalanceCallback);
        }
    }
    ...
    public function handleQuery(string $question): void
    {
        try {
            ...
            if (preg_match('/\b(cek\s+cashflow|cashflow|arus\s+kas|cash\s+flow)\b/i', $questionLower)) {
            ...
            try {
                ...
                if (preg_match('/pemasukan/i', $questionLower)) {
